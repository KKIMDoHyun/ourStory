...
import { createComment, fetchAllComments } from '@/api/comments';
import store from '.';
...
export default {
	...
	async FETCH_ALLCOMMENTS({ commit }, roomId) {
		const { data } = await fetchAllComments(roomId);
		commit('setAllComments', data);
		return data;
	},
	async CREATE_COMMENT({ commit }, commentData) {
